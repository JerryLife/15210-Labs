...
fun MST (E : edge seq, n : int) : edge seq = 
  let
    ...
    val labeled = map (fn (s,d,w)=>(s,(d,(s,d,w)))) sorted
    val verts = tabulate (fn i => i) n
    fun helper (Vs,Es,T) s =
    let
      val coins = Rand.flip s n
      fun hToT (s,(d,l)) = (nth coins s = 0) andalso (nth coins d = 1)
      val minEs = filter (fn (_,(a,_))=>a >= 0) (enum (inject Es (tabulate (fn _ => (~1,(~1,~1,~1))) n)))
      val contractions = filter hToT minEs
      val conts = map (fn (s,(d,_))=>(s,d)) contractions
      val P = inject conts Vs
      val T' = append ((map (fn (_,(_,l)) => l) contractions),T)
      val E' = map (fn (s,(d,l)) => ((nth P s),(nth P d,l))) Es
