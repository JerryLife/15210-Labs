...
struct
  ...
  fun findSegments (E, n) initial_credit =
    ...
    let
      ...
      val minCreds = map (fn (d,p) => (d,reduce Int.min initial_credit (map (fn (s,w)=>nth C s - w) p))) revConts
    ...
    val res = helper (verts,(map (fn (s,d,w) => (s,(d,w))) sorted),creds) (Rand.fromInt 15210)
  in
